...
prompt "This script will install fioncat/spacenvim to:"
echo "${DEST_DIR}"
...
if [[ "$USE_SSH" -eq "1" ]]; then
	if is_latest; then
		execute "git" "clone" "${CLONE_ATTR[@]}" "git@github.com:fioncat/spacenvim.git" "${DEST_DIR}"
	else
		abort "You have outdated Nvim installed (< ${REQUIRED_NVIM_VERSION})."
	fi
else
	if is_latest; then
		execute "git" "clone" "${CLONE_ATTR[@]}" "https://github.com/fioncat/spacenvim.git" "${DEST_DIR}"
	else
		abort "You have outdated Nvim installed (< ${REQUIRED_NVIM_VERSION})."
	fi
fi
...
prompt "If lazy.nvim failed to fetch any plugin(s), manually execute \`:Lazy sync\` until everything is up-to-date."
cat <<EOS

Thank you for using this set of configuration!
- Project Homepage:
    ${tty_underline}https://github.com/fioncat/spacenvim${tty_reset}
EOS
